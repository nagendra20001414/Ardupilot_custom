...
void AP_MotorsMatrix::check_failing_motors12()
{
    if (_thrust_rpyt_out[0] > (1 - _thr_warn_threshold) && _thrust_rpyt_out[1] < thr_remaining && _thrust_rpyt_out[2] < thr_remaining && _thrust_rpyt_out[3] < thr_remaining && !(AP_Notify::flags.is_mode_mf12)){
        if (counter1 == num_cycles_check){
            //setup mf12 mode and initialize all counters
            copter.set_mode_mf12();
            AP_Notify::events.failsafe_mode_change = 1; //alert pilot to mode change
    ...
    if (_thrust_rpyt_out[1] > (1 - _thr_warn_threshold) && _thrust_rpyt_out[0] < thr_remaining && _thrust_rpyt_out[2] < thr_remaining && _thrust_rpyt_out[3] < thr_remaining && !(AP_Notify::flags.is_mode_mf12)){
        if (counter2 == num_cycles_check){
            //setup mf12 mode and initialize all counters
            copter.set_mode_mf12();
            AP_Notify::events.failsafe_mode_change = 1; //alert pilot to mode change
            counter1 = 0; counter2 = 0; counter3 = 0; counter4 = 0;
...
void AP_MotorsMatrix::output_to_motors()
{
    ...
    switch (_spool_state) {
        ...
        case SpoolState::GROUND_IDLE:
            ...
            if (AP_Notify::flags.is_mode_mf12 && _last_frame_class == MOTOR_FRAME_QUAD && _last_frame_type == MOTOR_FRAME_TYPE_X){
                if (motor_enabled[3]){
                set_actuator_with_slew(_actuator[3],
                                       actuator_spin_up_to_ground_idle());
                }
                if (motor_enabled[2]){
                set_actuator_with_slew(_actuator[2],
                                       actuator_spin_up_to_ground_idle());
                }
                _actuator[1] = 0.0f;
                _actuator[0] = 0.0f;
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            if (AP_Notify::flags.is_mode_mf12 && _last_frame_class == MOTOR_FRAME_QUAD && _last_frame_type == MOTOR_FRAME_TYPE_X){
                if (motor_enabled[3]){
                    set_actuator_with_slew(_actuator[3],
                                           thrust_to_actuator(_thrust_rpyt_out[3] + _thrust_rpyt_out[1]));
                }
                if (motor_enabled[2]){
                    set_actuator_with_slew(_actuator[2],
                                           thrust_to_actuator(_thrust_rpyt_out[2] + _thrust_rpyt_out[0]));
                }
                _actuator[1] = 0.0f;
                _actuator[0] = 0.0f;
                ...
                if (AP_Notify::flags.is_mode_mfi1 && _last_frame_class == MOTOR_FRAME_QUAD && _last_frame_type == MOTOR_FRAME_TYPE_X){
                    set_actuator_with_slew(_actuator[0], thrust_to_actuator(_thrust_rpyt_out[0]/4));
                    set_actuator_with_slew(_actuator[1], thrust_to_actuator(_thrust_rpyt_out[1]));
                    set_actuator_with_slew(_actuator[2], thrust_to_actuator(_thrust_rpyt_out[2]));
                    set_actuator_with_slew(_actuator[3], thrust_to_actuator(_thrust_rpyt_out[3]));
                }else{
